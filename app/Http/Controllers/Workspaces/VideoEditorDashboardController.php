...
class VideoEditorDashboardController extends Controller
{
     ...
     * All sections respect the date / product / format filters; the leaderboard
     * intentionally ignores the editor scope so an editor can see where they
     * rank against the rest of the team.
     */
    public function __invoke(Request $request, Workspace $workspace): Response
    {
        ...
        // Throughput grouping (daily / weekly / monthly / yearly), clamped to
        // the options that make sense for the selected range — mirrors the main
        // dashboard's breakdown toggle.
        $group = $this->resolveGroup($request->input('group'), $dateFrom, $dateTo);
        ...
        // ─── Throughput (grouped buckets, video vs image) ──────────────────
        $throughput = $this->throughput($creatives, $dateFrom, $dateTo, $group);

        // ─── Leaderboard (all editors, ignores the editor scope) ───────────
     ...
     * @return list<string>
     */
    private function availableGroups(CarbonImmutable $dateFrom, CarbonImmutable $dateTo): array
    {
        $days = $dateFrom->diffInDays($dateTo);

        return match (true) {
            $days <= 7 => ['daily'],
            $days <= 30 => ['daily', 'weekly'],
            $days <= 365 => ['weekly', 'monthly'],
     ...
     */
    private function resolveGroup(?string $group, CarbonImmutable $dateFrom, CarbonImmutable $dateTo): string
    {
        $available = $this->availableGroups($dateFrom, $dateTo);
     ...
     */
    private function throughput($creatives, CarbonImmutable $dateFrom, CarbonImmutable $dateTo, string $group): array
    {
        // Bucket size follows the chosen grouping; labels stay readable per unit.
        [$labelFmt, $floor, $advance] = match ($group) {
        ...
        // Build ordered buckets across the range, keyed by the bucket start.
        $buckets = [];
        $cursor = $floor($dateFrom);
        $end = $floor($dateTo);
        while ($cursor <= $end) {
            $buckets[$cursor->format('Y-m-d')] = ['label' => $cursor->format($labelFmt), 'video' => 0, 'image' => 0];
        ...
        foreach ($creatives as $c) {
            ...
            $key = $floor(CarbonImmutable::parse($c->creative_date))->format('Y-m-d');
