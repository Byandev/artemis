...
class ForDeliveryController extends Controller
{
    ...
    public function analyticsTopAssignees(Workspace $workspace): JsonResponse
    {
        $data = OrderForDelivery::query()
            ->from('pancake_order_for_delivery as ofd')
            ->join('pancake_users as u', 'u.id', '=', 'ofd.conferrer_id')
            ->where('ofd.workspace_id', $workspace->id)
            ->whereNotNull('ofd.conferrer_id')
            ->selectRaw('u.id, u.name, COUNT(*) as total_orders')
            ->groupBy('u.id', 'u.name')
    ...
    public function analyticsStatusBreakdown(Workspace $workspace): JsonResponse
    {
        $data = OrderForDelivery::query()
            ->where('workspace_id', $workspace->id)
    ...
    public function analyticsDeliveryTrend(Workspace $workspace): JsonResponse
    {
        $data = OrderForDelivery::query()
            ->where('workspace_id', $workspace->id)
    ...
    public function analyticsPerPage(Workspace $workspace): JsonResponse
    {
        $data = OrderForDelivery::query()
            ->from('pancake_order_for_delivery as ofd')
            ->join('pages as p', 'p.id', '=', 'ofd.page_id')
            ->where('ofd.workspace_id', $workspace->id)
            ->selectRaw('p.id, p.name, COUNT(*) as total_orders')
            ->groupBy('p.id', 'p.name')
            ->orderByDesc('total_orders')
            ->limit(10)
            ->get();
