...
|--------------------------------------------------------------------------
|
| The one part of the page not narrowed to the reader: a board ranking a roster
| of one would crown them on every figure and say nothing. Same rankings the
| analytics page shows (App\Support\CsrLeaders), drawn from every shop in the
| workspace rather than from the ones a team may see — team scoping fails
| closed for a CSR in no team, and an empty board would break the page for
| exactly the people it is for.
*/

it('crowns the workspace\'s top seller, not the reader', function () {
    [$member, $mine] = dashMember($this->workspace, $this->shop->id);
    $theirs = dashCsr($this->shop->id, 'Theirs');

    dashPos($this->workspace, $this->shop->id, $mine, '2026-08-10', 1000, 5);
    dashPos($this->workspace, $this->shop->id, $theirs, '2026-08-10', 3000, 10);

    dashCard($member, $this->workspace, 'leader-sales')
        ->assertOk()
        ->assertJsonPath('leader.name', 'Theirs')
        ->assertJsonPath('leader.value', 3000)
        ->assertJsonPath('leader.orders', 10)
        // 3000 of the 4000 the two of them confirmed between them.
        ->assertJsonPath('leader.share', 75);
});

it('ranks a CSR who is in no team at all', function () {
    // The reader's own dashboard must not fail closed the way the analytics
    // page does for a scoped viewer with no team — there is no team here.
    [$member, $mine] = dashMember($this->workspace, $this->shop->id);
    dashPos($this->workspace, $this->shop->id, $mine, '2026-08-10', 1000, 5);

    dashCard($member, $this->workspace, 'leader-sales')
        ->assertJsonPath('leader.name', 'Mine');
});

it('crowns the lowest RTS rate rather than the highest', function () {
    [$member, $mine] = dashMember($this->workspace, $this->shop->id);
    $theirs = dashCsr($this->shop->id, 'Theirs');

    // 10% back against 30% back — low is the win on this one card.
    dashPos($this->workspace, $this->shop->id, $mine, '2026-08-10', 1000, 5, [
        'returning' => 100,
        'delivered' => 900,
    ]);
    dashPos($this->workspace, $this->shop->id, $theirs, '2026-08-10', 1000, 5, [
        'returning' => 300,
        'delivered' => 700,
    ]);

    dashCard($member, $this->workspace, 'leader-rts')
        ->assertJsonPath('leader.name', 'Mine')
        ->assertJsonPath('leader.value', 10);
});

it('crowns the RMO leaders off the call rollup', function () {
    [$member, $mine] = dashMember($this->workspace, $this->shop->id);
    $theirs = dashCsr($this->shop->id, 'Theirs');

    dashCall($this->workspace, $this->shop->id, $mine, '2026-08-10', [
        'total_rmo_assigned_count' => 40,
        'total_rmo_confirmed_count' => 50,
        'total_rmo_call_time' => 900,
        'total_rmo_called' => 30,
    ]);
    dashCall($this->workspace, $this->shop->id, $theirs, '2026-08-10', [
        'total_rmo_assigned_count' => 45,
        'total_rmo_confirmed_count' => 50,
        'total_rmo_call_time' => 600,
        'total_rmo_called' => 20,
    ]);

    // 45/50 beats 40/50.
    dashCard($member, $this->workspace, 'leader-rmo-called')
        ->assertJsonPath('leader.name', 'Theirs')
        ->assertJsonPath('leader.value', 90);

    // But the most time on the phone is the other one.
    dashCard($member, $this->workspace, 'leader-rmo-duration')
        ->assertJsonPath('leader.name', 'Mine')
        ->assertJsonPath('leader.value', 900)
        ->assertJsonPath('leader.calls', 30)
        ->assertJsonPath('leader.average_seconds', 30);
});

it('crowns nobody on a quiet period', function () {
    [$member] = dashMember($this->workspace, $this->shop->id);

    foreach (['leader-sales', 'leader-rts', 'leader-rmo-called', 'leader-rmo-duration'] as $card) {
        dashCard($member, $this->workspace, $card)
            ->assertOk()
...
it('gates the leader cards the same way as the stat cards', function () {
    $cards = ['leader-sales', 'leader-rts', 'leader-rmo-called', 'leader-rmo-duration'];
