<?php

namespace Database\Seeders;

use App\Models\SubscriptionPlan;
use Illuminate\Database\Seeder;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\DB;

class SubscriptionPlanSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $now = Carbon::now();

        $plans = [
            [
                'code' => SubscriptionPlan::CODE_FREE_TRIAL,
                'name' => 'Free Trial',
                'price_php' => 0,
                'order_limit' => 10000,
                'shop_limit' => 1,
                'data_retention_months' => 6,
                'analytics_tier' => SubscriptionPlan::ANALYTICS_FULL,
                'parcel_journey_rate_php' => null,
                'parcel_journey_sms_enabled' => true,
                'support_tier' => SubscriptionPlan::SUPPORT_CHAT,
                'trial_days' => 30,
                'is_active' => true,
                'sort_order' => 1,
            ],
            [
                'code' => SubscriptionPlan::CODE_STARTER, // internal code unchanged; display name is now "Solo"
                'name' => 'Solo',
                'price_php' => 4499,
                'order_limit' => 3000,
                'shop_limit' => 5,
                'data_retention_months' => 3,
                'analytics_tier' => SubscriptionPlan::ANALYTICS_BASIC,
                'parcel_journey_rate_php' => null,
                'parcel_journey_sms_enabled' => true,
                'support_tier' => SubscriptionPlan::SUPPORT_PRIORITY_CHAT,
                'trial_days' => null,
                'is_active' => true,
                'sort_order' => 2,
            ],
            [
                'code' => SubscriptionPlan::CODE_GROWTH, // internal code unchanged; display name is now "Pro"
                'name' => 'Pro',
                'price_php' => 8999,
                'order_limit' => 10000,
                'shop_limit' => 25,
                'data_retention_months' => 6,
                'analytics_tier' => SubscriptionPlan::ANALYTICS_FULL,
                'parcel_journey_rate_php' => null,
                'parcel_journey_sms_enabled' => true,
                'support_tier' => SubscriptionPlan::SUPPORT_PRIORITY_CHAT,
                'trial_days' => null,
                'is_active' => true,
                'sort_order' => 3,
            ],
            [
                'code' => SubscriptionPlan::CODE_SCALE, // internal code unchanged; display name is now "Business"
                'name' => 'Business',
                'price_php' => 13499,
                'order_limit' => 20000,
                'shop_limit' => 100,
                'data_retention_months' => 12,
                'analytics_tier' => SubscriptionPlan::ANALYTICS_FULL,
                'parcel_journey_rate_php' => null,
                'parcel_journey_sms_enabled' => true,
                'support_tier' => SubscriptionPlan::SUPPORT_DEDICATED,
                'trial_days' => null,
                'is_active' => true,
                'sort_order' => 4,
            ],
            [
                'code' => SubscriptionPlan::CODE_ENTERPRISE,
                'name' => 'Enterprise',
                'price_php' => 0, // Custom — negotiated per customer; UI displays "Custom"
                'order_limit' => null, // unlimited
                'shop_limit' => null,  // unlimited
                'data_retention_months' => 24,
                'analytics_tier' => SubscriptionPlan::ANALYTICS_FULL,
                'parcel_journey_rate_php' => null,
                'parcel_journey_sms_enabled' => true,
                'support_tier' => SubscriptionPlan::SUPPORT_DEDICATED,
                'trial_days' => null,
                'is_active' => true,
                'sort_order' => 5,
            ],
        ];

        foreach ($plans as $plan) {
            DB::table('subscription_plans')->updateOrInsert(
                ['code' => $plan['code']],
                [
                    ...$plan,
                    'created_at' => $now,
                    'updated_at' => $now,
                ]
            );
        }
    }
}
