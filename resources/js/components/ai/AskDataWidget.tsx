...
const SECTIONS: AskWidgetSection[] = [
    {
        ...
        defaultQuestion:
            'Analyze my metrics. What is working and what needs attention?',
    },
    {
        key: 'pages',
        ...
        defaultQuestion:
            'Which pages are performing well and which are struggling?',
    },
    {
        key: 'shops',
        ...
        switch (key) {
            case 'metrics': {
                ...
                Object.entries(data.metrics).forEach(([k, v]) => {
                    ...
                    formatted[config?.name ?? k] = config
                        ? config.formatter(v)
                        : String(v);
                });
            ...
            case 'pages':
                return {
                    period,
                    metric:
                        metricConfigs.find((m) => m.key === data.pages.metric)
                            ?.name ?? data.pages.metric,
                    pages: formatBreakdown(data.pages),
                };
            case 'shops':
                return {
                    period,
                    metric:
                        metricConfigs.find((m) => m.key === data.shops.metric)
                            ?.name ?? data.shops.metric,
                    shops: formatBreakdown(data.shops),
                };
            case 'users':
                return {
                    period,
                    metric:
                        metricConfigs.find((m) => m.key === data.users.metric)
                            ?.name ?? data.users.metric,
                    users: formatBreakdown(data.users),
                };
            default:
                return { period };
        }
