...
const WorkspaceSwitcher = () => {
    ...
    const [rmoPwOpen, setRmoPwOpen] = useState(false);
    const rmoPwForm = useForm({ password: '' });

    const submitRmoPassword = (e: FormEvent) => {
        e.preventDefault();
        if (!currentWorkspace) return;
        rmoPwForm.post(
            `/workspaces/${currentWorkspace.slug}/rmo-public-password`,
            {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('RMO public access password saved.');
                    rmoPwForm.reset();
                    setRmoPwOpen(false);
                },
    ...
    const removeRmoPassword = () => {
        ...
        router.post(
            ...
            { password: '' },
            {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('RMO public access password removed.');
                    rmoPwForm.reset();
                    setRmoPwOpen(false);
                },
    ...
    return (
        <>
                            ...
                            <div className="space-y-0.5 px-2 py-2">
                                ...
                                {canEditSettings && (
                                    ...
                                    >
                                        ...
                                        RMO Public Access
                                        {currentWorkspace.rmo_public_password_set && (
            ...
            <Dialog open={rmoPwOpen} onOpenChange={setRmoPwOpen}>
                <DialogContent>
                    <form onSubmit={submitRmoPassword}>
                        <DialogHeader>
                            <DialogTitle>
                                RMO Public Access Password
                            </DialogTitle>
                            <DialogDescription>
                                ...
                                . Leave blank and remove to make it open again.
                            </DialogDescription>
                        </DialogHeader>

                        <div className="space-y-2 py-4">
                            <Label htmlFor="rmo-public-password">
                                {currentWorkspace.rmo_public_password_set
                                    ? 'New password'
                                    : 'Password'}
                            </Label>
                            <Input
                                id="rmo-public-password"
                                type="password"
                                autoComplete="new-password"
                                placeholder="Enter a password (min 4 characters)"
                                value={rmoPwForm.data.password}
                                onChange={(e) =>
                                    rmoPwForm.setData(
                                        'password',
                                        e.target.value,
                            ...
                            />
                            <InputError message={rmoPwForm.errors.password} />
                            {currentWorkspace.rmo_public_password_set && (
                        ...
                        <DialogFooter className="gap-2 sm:justify-between">
                            {currentWorkspace.rmo_public_password_set ? (
                                <Button
                                    type="button"
                                    variant="outline"
                                    onClick={removeRmoPassword}
                                    disabled={rmoPwForm.processing}
                            ...
                            <Button
                                type="submit"
                                disabled={
                                    rmoPwForm.processing ||
                                    !rmoPwForm.data.password
                                }
                            >
                                {rmoPwForm.processing ? 'Saving…' : 'Save'}
                            </Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
