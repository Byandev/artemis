import RefreshButton from '@/components/inventory/dashboard/refresh-button';
import { Skeleton } from '@/components/ui/skeleton';
import { cn } from '@/lib/utils';
import { useMemo } from 'react';
import { formatKpi } from './kpi-card';
import { useSalesMarketingStat } from './use-sales-marketing-stat';

/** One advertiser's raw sums for the window, as the endpoint answers. */
interface BreakdownRow {
    advertiser: { id: number; name: string | null };
    ad_spend: number;
    sales: number;
    orders: number;
    returned_amount: number;
    delivered_amount: number;
}

interface Breakdown {
    rows: BreakdownRow[];
}

/**
 * Where a figure stops reading as healthy. Both are house rules rather than
 * anything the data implies, so they live here as named constants — a ROAS
 * under 3× and an RTS over 20% are the two the table calls out in red.
 */
const ROAS_TARGET = 3;
const RTS_CEILING = 0.2;

/** How many members are visible before the table starts scrolling. */
const VISIBLE_ROWS = 10;

/**
 * Row height in rem, fixed so the scroll cap can be stated in rows rather than
 * guessed in pixels. Header and footer are the same height, hence the +2.
 */
const ROW_H = 2.5;

/** Sticky cells need an opaque background, or scrolled rows show through. */
const STICKY_BG = 'bg-white dark:bg-zinc-900';

/**
 * The per-member table beneath the comparison chart: the same rows, stated
 * rather than plotted, with a sub-total.
 *
 * Every derived figure — ROAS, RTS rate, and the whole sub-total row — is worked
 * out here from the raw sums. The sub-total's ROAS and RTS are blended (totals
 * divided), not means of the columns above: averaging six members' ratios would
 * weight a member with 4,000 orders the same as one with 24,000.
 */
export default function TeamBreakdown({
    slug,
    dateRange,
}: {
    slug: string;
    /** `[start, end]` as YYYY-MM-DD. */
    dateRange: string[];
}) {
    const { data, loading, error, refetch } = useSalesMarketingStat<Breakdown>(
        slug,
        'team-breakdown',
        { start: dateRange[0], end: dateRange[1] },
    );

    const rows = useMemo(
        () =>
            (data?.rows ?? [])
                .map((row) => {
                    const outcomes = row.returned_amount + row.delivered_amount;

                    return {
                        ...row,
                        id: row.advertiser.id,
                        name:
                            row.advertiser.name?.trim() || 'Unnamed advertiser',
                        roas:
                            row.ad_spend > 0 ? row.sales / row.ad_spend : null,
                        rts:
                            outcomes > 0
                                ? row.returned_amount / outcomes
                                : null,
                    };
                })
                .sort((a, b) => b.sales - a.sales),
        [data],
    );

    const total = useMemo(() => {
        const sum = (pick: (r: BreakdownRow) => number) =>
            rows.reduce((acc, r) => acc + pick(r), 0);

        const sales = sum((r) => r.sales);
        const adSpend = sum((r) => r.ad_spend);
        const returned = sum((r) => r.returned_amount);
        const outcomes = returned + sum((r) => r.delivered_amount);

        return {
            orders: sum((r) => r.orders),
            sales,
            adSpend,
            returned,
            // Blended, not a mean of the rows above.
            roas: adSpend > 0 ? sales / adSpend : null,
            rts: outcomes > 0 ? returned / outcomes : null,
        };
    }, [rows]);

    // Skeleton until the first answer; a later refetch dims rather than flashing.
    const firstLoad = loading && !data;

    return (
        <section className="mt-10">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-[11px] font-medium tracking-wide text-gray-400 uppercase dark:text-gray-500">
                    Breakdown per team member
                </h2>
                <RefreshButton
                    onClick={refetch}
                    loading={loading}
                    error={error}
                    label="the team breakdown"
                />
            </div>

            <div className="rounded-[14px] border border-black/6 bg-white dark:border-white/6 dark:bg-zinc-900">
                {firstLoad ? (
                    <div className="space-y-3 p-[18px]">
                        {Array.from({ length: 6 }).map((_, i) => (
                            <Skeleton key={i} className="h-5 w-full" />
                        ))}
                    </div>
                ) : error ? (
                    <p className="p-[18px] text-[11px] text-red-500 dark:text-red-400">
                        Failed to load — retry with the refresh button above.
                    </p>
                ) : rows.length === 0 ? (
                    <p className="p-[18px] text-[11px] text-gray-400 dark:text-gray-500">
                        nothing to break down in this period
                    </p>
                ) : (
                    // Scrolls in its own container both ways: wider than the
                    // card on small screens, and taller than VISIBLE_ROWS once
                    // the team grows. Rows are a fixed height so that cap is
                    // exactly ten of them plus the pinned header and footer.
                    <div
                        className={cn(
                            'overflow-auto rounded-[14px] transition-opacity',
                            loading && 'opacity-50',
                        )}
                        style={{
                            maxHeight: `${(VISIBLE_ROWS + 2) * ROW_H}rem`,
                        }}
                    >
                        {/* border-separate, not collapse: a collapsed table
                            drops the borders of sticky cells as they move. */}
                        <table className="w-full min-w-[52rem] border-separate border-spacing-0 text-[12px]">
                            <thead>
                                <tr>
                                    <Th align="left" sticky>
                                        Team member
                                    </Th>
                                    <Th>Orders</Th>
                                    <Th>Sales</Th>
                                    <Th>Ad spend</Th>
                                    <Th>ROAS</Th>
                                    <Th>RTS rate</Th>
                                    <Th>RTS amount</Th>
                                </tr>
                            </thead>

                            <tbody>
                                {rows.map((r) => (
                                    <tr key={r.id}>
                                        <Td align="left" sticky>
                                            <span
                                                className="block truncate font-mono text-gray-700 uppercase dark:text-gray-200"
                                                title={r.name}
                                            >
                                                {r.name}
                                            </span>
                                        </Td>
                                        <Td>{formatKpi(r.orders, 'number')}</Td>
                                        <Td>
                                            {formatKpi(
                                                r.sales,
                                                'currencyExact',
                                            )}
                                        </Td>
                                        <Td>
                                            {formatKpi(
                                                r.ad_spend,
                                                'currencyExact',
                                            )}
                                        </Td>
                                        {/* Colour is emphasis, never the only
                                            carrier — the figure itself is
                                            readable without it. */}
                                        <Td
                                            className={
                                                r.roas === null
                                                    ? undefined
                                                    : r.roas >= ROAS_TARGET
                                                      ? 'font-semibold text-emerald-600 dark:text-emerald-400'
                                                      : 'font-semibold text-red-600 dark:text-red-400'
                                            }
                                        >
                                            {r.roas === null
                                                ? '—'
                                                : formatKpi(r.roas, 'ratio')}
                                        </Td>
                                        <Td
                                            className={
                                                r.rts !== null &&
                                                r.rts > RTS_CEILING
                                                    ? 'font-semibold text-red-600 dark:text-red-400'
                                                    : undefined
                                            }
                                        >
                                            {r.rts === null
                                                ? '—'
                                                : formatKpi(r.rts, 'percent')}
                                        </Td>
                                        <Td className="text-gray-500 dark:text-gray-400">
                                            {formatKpi(
                                                r.returned_amount,
                                                'currencyExact',
                                            )}
                                        </Td>
                                    </tr>
                                ))}
                            </tbody>

                            <tfoot>
                                <tr className="font-semibold italic">
                                    <Td align="left" sticky foot>
                                        Sub-Total
                                    </Td>
                                    <Td foot>
                                        {formatKpi(total.orders, 'number')}
                                    </Td>
                                    <Td foot>
                                        {formatKpi(
                                            total.sales,
                                            'currencyExact',
                                        )}
                                    </Td>
                                    <Td foot>
                                        {formatKpi(
                                            total.adSpend,
                                            'currencyExact',
                                        )}
                                    </Td>
                                    <Td foot>
                                        {total.roas === null
                                            ? '—'
                                            : formatKpi(total.roas, 'ratio')}
                                    </Td>
                                    <Td foot>
                                        {total.rts === null
                                            ? '—'
                                            : formatKpi(total.rts, 'percent')}
                                    </Td>
                                    <Td foot>
                                        {formatKpi(
                                            total.returned,
                                            'currencyExact',
                                        )}
                                    </Td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                )}
            </div>
        </section>
    );
}

/**
 * Header cell, pinned to the top of the scroll container. The first column is
 * pinned left as well, so it outranks the others where the two overlap.
 */
function Th({
    children,
    align = 'right',
    sticky,
}: {
    children: React.ReactNode;
    align?: 'left' | 'right';
    /** Pin this column horizontally — the team member name. */
    sticky?: boolean;
}) {
    return (
        <th
            scope="col"
            style={{ height: `${ROW_H}rem` }}
            className={cn(
                'sticky top-0 z-20 border-b border-black/6 px-4 font-mono text-[10px] font-medium tracking-wider text-gray-400 uppercase dark:border-white/6 dark:text-gray-500',
                STICKY_BG,
                align === 'left' ? 'text-left' : 'text-right',
                sticky &&
                    'left-0 z-30 border-r border-black/6 dark:border-white/6',
            )}
        >
            {children}
        </th>
    );
}

function Td({
    children,
    align = 'right',
    className,
    sticky,
    foot,
}: {
    children: React.ReactNode;
    align?: 'left' | 'right';
    className?: string;
    /** Pin this column horizontally — the team member name. */
    sticky?: boolean;
    /** A sub-total cell: pinned to the bottom so it survives scrolling. */
    foot?: boolean;
}) {
    return (
        <td
            style={{ height: `${ROW_H}rem` }}
            className={cn(
                'px-4 font-mono text-gray-700 tabular-nums dark:text-gray-200',
                foot
                    ? cn(
                          'sticky bottom-0 z-20 border-t border-black/8 dark:border-white/10',
                          STICKY_BG,
                      )
                    : 'border-b border-black/4 dark:border-white/4',
                align === 'left' ? 'text-left' : 'text-right',
                sticky &&
                    cn(
                        'sticky left-0 w-56 max-w-56 border-r border-black/6 dark:border-white/6',
                        STICKY_BG,
                        foot ? 'z-30' : 'z-10',
                    ),
                className,
            )}
        >
            {children}
        </td>
    );
}
