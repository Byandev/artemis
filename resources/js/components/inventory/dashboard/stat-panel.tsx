...
import { type StatState } from './use-inventory-stat';

interface Props<T> {
    title: string;
    icon?: ReactNode;
    /** Right-aligned header content (e.g. a toggle), placed before refresh. */
    action?: ReactNode;
    count?: number;
    className?: string;
    state: StatState<T>;
...
/**
 * Themed dashboard panel matching the video-editor dashboard's Panel, with a
 * built-in refresh button and an error state that offers a retry — so a failed
 * widget never breaks the page and can be recovered on its own.
 */
export default function StatPanel<T>({
    ...
    return (
        ...
        >
            ...
            <div className="inv-chart-panel flex-1 p-4 sm:p-5">
                {error ? (
