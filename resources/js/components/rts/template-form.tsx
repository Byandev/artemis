...
const TemplateForm = ({
    ...
    const insertAtCursor = useCallback(
        (placeholder: string) => {
            ...
            const newText =
                data.message.slice(0, start) +
                placeholder +
                data.message.slice(end);
    ...
    const variables = useMemo(
        () => [
            ...
            { display: 'Barangay', code: '{{barangay}}', isVisible: true },
            {
                ...
                code: '{{next_location}}',
                isVisible: initialValue?.activity === 'departure',
            },
            {
                display: 'Current Location',
                code: '{{current_location}}',
                isVisible: initialValue?.activity === 'arrival',
            },
            {
                display: 'Rider Name',
                code: '{{rider_name}}',
                isVisible: initialValue?.activity === 'for-delivery',
            },
            {
                display: 'Rider Mobile',
                code: '{{rider_mobile}}',
                isVisible: initialValue?.activity === 'for-delivery',
            },
        ],
                    ...
                    <div className="space-y-4 px-5 py-4">
                        <div className="flex items-center justify-between rounded-[10px] border border-black/8 bg-stone-50 px-3 py-2.5 dark:border-white/8 dark:bg-zinc-800">
                            <div>
                                ...
                                <p className="font-mono text-[10px] text-gray-400 dark:text-gray-500">
                                    Notifications for this template are only
                                    sent when enabled.
                                </p>
                            </div>
                            <Switch
                        ...
                        <div className="space-y-1.5">
                            ...
                            <textarea
                                ...
                                className={`w-full resize-none rounded-[10px] border px-3 py-2.5 font-mono! text-[12px]! transition-all outline-none ${
                                    errors.message
                                        ? 'border-red-500 text-red-900 focus:border-red-500 focus:ring-2 focus:ring-red-500/15 dark:text-red-200'
                            ...
                            {errors.message ? (
                                <p className="animate-in fade-in slide-in-from-top-1 font-mono text-[10px] font-medium text-red-500">
                                    {errors.message}
                                </p>
                            ) : (
                                <p className="font-mono text-[10px] text-gray-400 dark:text-gray-500">
                                    For SMS, keep the message as short as
                                    possible.
                                </p>
                            )}
                        </div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {variables
                                    ...
                                    .map((variable) => (
                                        ...
                                        >
                                            <span className="font-mono text-[10px] font-semibold text-violet-400 dark:text-violet-500">
                                                {'{ }'}
                                            </span>
                                            <span className="font-mono text-[11px] font-medium text-violet-700 dark:text-violet-300">
                                                {variable.display}
                                            </span>
                                        </button>
                                    ))}
