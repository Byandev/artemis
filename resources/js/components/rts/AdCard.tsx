...
export default function AdCard({ workspaceSlug, queryParams }: Props) {
    const [data, setData] = useState<PaginatedData<AdRow> | null>(null);
    ...
    const fetchPage = (page: number, currentSort: string, perPage = 15) => {
        ...
        fetch(`/workspaces/${workspaceSlug}/rts/analytics/group-by/ad?${p}`, { credentials: 'same-origin' })
    ...
    const columns: ColumnDef<AdRow>[] = useMemo(() => [
        {
            ...
            cell: ({ row }) => row.original.ad_name ?? <span className="">{row.original.ad_id ?? 'Unknown'}</span>,
        },
        {
                ...
                {loading ? (
                    ...
                    <RtsBreakdownChart
                        rows={(data?.data ?? []).map((r) => ({
                            label: r.ad_name ?? r.ad_id ?? 'Unknown',
