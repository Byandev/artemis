...
export default function ConfirmedByCard({ workspaceSlug, queryParams }: Props) {
    const [data, setData] = useState<PaginatedData<ConfirmedByRow> | null>(
        null,
    );
    ...
    const fetchPage = (page: number, currentSort: string, perPage = 10) => {
        ...
        fetch(
            `/workspaces/${workspaceSlug}/rts/analytics/group-by/confirmed-by?${p}`,
        ...
        () => [
            {
                ...
                cell: ({ row }) =>
                    row.original.confirmed_by_name ?? (
            ...
            <div className="flex items-center justify-between border-b border-black/6 px-5 py-4 dark:border-white/6">
                <div>
                    ...
                    <p className="mt-0.5 text-[12px] text-gray-400 dark:text-gray-500">
                        RTS rate broken down by confirming agent
                    </p>
                </div>
