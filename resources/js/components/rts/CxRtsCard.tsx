...
export default function CxRtsCard({
    workspaceSlug,
    queryParams,
    onDataLoaded,
}: Props) {
    ...
    useEffect(() => {
        ...
        p.append('type', type);
        fetch(
            `/workspaces/${workspaceSlug}/rts/analytics/group-by/cx-rts?${p}`,
    ...
    return (
        <div className="rounded-2xl border border-black/6 bg-white dark:border-white/6 dark:bg-zinc-900">
            <div className="flex flex-col gap-3 border-b border-black/6 px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/6">
                <div className="min-w-0">
                    <h2 className="text-[14px] font-semibold text-gray-900 dark:text-gray-100">
                        By Customer RTS (Phone Number Report)
                    </h2>
