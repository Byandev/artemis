import { Checkbox } from '@/components/ui/checkbox';
import {
    Collapsible,
    CollapsibleContent,
    CollapsibleTrigger,
} from '@/components/ui/collapsible';
import {
    Popover,
    PopoverContent,
    PopoverTrigger,
} from '@/components/ui/popover';
import { Workspace } from '@/types/models/Workspace';
import { ChevronDown, ChevronUp, SlidersHorizontal, X } from 'lucide-react';
import { useCallback, useMemo, useState } from 'react';

type IdLike = string | number;

export type RtsFilterValue = {
    teamIds: IdLike[];
    shopIds: IdLike[];
    pageIds: IdLike[];
};

type FilterOption = {
    id: IdLike;
    name: string;
};

type FilterGroupProps = {
    name: string;
    options: FilterOption[];
    selected: IdLike[];
    onSelect: (id: IdLike) => void;
};

const EMPTY_FILTER_VALUE: RtsFilterValue = {
    teamIds: [],
    shopIds: [],
    pageIds: [],
};

const countActiveFilters = (value: RtsFilterValue) =>
    value.teamIds.length + value.shopIds.length + value.pageIds.length;

const hasId = (ids: IdLike[], id: IdLike) =>
    ids.map(String).includes(String(id));

function RtsFilterGroup({ name, options, selected, onSelect }: FilterGroupProps) {
    const [open, setOpen] = useState(name === 'TEAM');
    const activeCount = selected.length;

    return (
        <Collapsible open={open} onOpenChange={setOpen}>
            <CollapsibleTrigger
                className={[
                    'flex w-full items-center justify-between rounded-lg px-2 py-2 transition-colors',
                    'hover:bg-black/[0.02] dark:hover:bg-white/[0.03]',
                ].join(' ')}
            >
                <span className="flex items-center gap-2">
                    <span className="font-mono text-[10px] font-medium uppercase tracking-wider text-gray-300 dark:text-gray-600">
                        {name}
                    </span>
                    {activeCount > 0 && (
                        <span className="inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-emerald-600 px-1 text-[10px] font-semibold text-white tabular-nums shadow-sm shadow-emerald-600/20 dark:bg-emerald-500">
                            {activeCount}
                        </span>
                    )}
                </span>
                {open ? (
                    <ChevronUp className="h-3.5 w-3.5 text-gray-300 dark:text-gray-600" />
                ) : (
                    <ChevronDown className="h-3.5 w-3.5 text-gray-300 dark:text-gray-600" />
                )}
            </CollapsibleTrigger>

            <CollapsibleContent className="mt-0.5">
                <div className="max-h-48 overflow-y-auto pr-1">
                    {options.length === 0 ? (
                        <p className="px-2 py-3 text-center text-[11px] text-gray-400 dark:text-gray-600">
                            No options
                        </p>
                    ) : (
                        options.map((option) => {
                            const checked = hasId(selected, option.id);
                            const checkboxId = `rts-filter-${name.toLowerCase()}-${option.id}`;

                            return (
                                <label
                                    key={String(option.id)}
                                    htmlFor={checkboxId}
                                    className={[
                                        'flex cursor-pointer items-center gap-2.5 rounded-[8px] px-2 py-2 transition-colors',
                                        checked
                                            ? 'bg-emerald-500/[0.07] dark:bg-emerald-500/[0.10]'
                                            : 'hover:bg-black/[0.02] dark:hover:bg-white/[0.03]',
                                    ].join(' ')}
                                >
                                    <Checkbox
                                        id={checkboxId}
                                        checked={checked}
                                        onCheckedChange={() => onSelect(option.id)}
                                        className="border-black/20 data-[state=checked]:border-emerald-600 data-[state=checked]:bg-emerald-600 dark:border-white/20 dark:data-[state=checked]:border-emerald-500 dark:data-[state=checked]:bg-emerald-500"
                                    />
                                    <span
                                        className={[
                                            'truncate text-[13px] select-none',
                                            checked
                                                ? 'font-medium text-emerald-700 dark:text-emerald-400'
                                                : 'text-gray-600 dark:text-gray-400',
                                        ].join(' ')}
                                    >
                                        {option.name}
                                    </span>
                                </label>
                            );
                        })
                    )}
                </div>
            </CollapsibleContent>
        </Collapsible>
    );
}

type Props = {
    workspace: Workspace;
    value: RtsFilterValue;
    onChange: (value: RtsFilterValue) => void;
};

export default function RtsAnalyticsFilters({ workspace, value, onChange }: Props) {
    const [open, setOpen] = useState(false);
    const [localValue, setLocalValue] = useState(value);
    const [hasChanges, setHasChanges] = useState(false);

    const activeCount = useMemo(() => countActiveFilters(localValue), [localValue]);
    const hasActiveFilters = activeCount > 0;

    const pageOptions = useMemo(
        () => (workspace.pages ?? []).map((page) => ({ id: page.id, name: page.name })),
        [workspace.pages],
    );
    const shopOptions = useMemo(
        () => (workspace.shops ?? []).map((shop) => ({ id: shop.id, name: shop.name })),
        [workspace.shops],
    );
    const teamOptions = useMemo(
        () => (workspace.teams ?? []).map((team) => ({ id: team.id, name: team.name })),
        [workspace.teams],
    );

    const handleSelect = useCallback((key: keyof RtsFilterValue, id: IdLike) => {
        setLocalValue((prev) => {
            const current = prev[key];
            const next = hasId(current, id)
                ? current.filter((item) => String(item) !== String(id))
                : [...current, id];

            return { ...prev, [key]: next };
        });
        setHasChanges(true);
    }, []);

    const handleClearAll = useCallback(() => {
        setLocalValue(EMPTY_FILTER_VALUE);
        onChange(EMPTY_FILTER_VALUE);
        setHasChanges(false);
    }, [onChange]);

    const handleApply = useCallback(() => {
        onChange(localValue);
        setHasChanges(false);
        setOpen(false);
    }, [localValue, onChange]);

    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <button
                    className={[
                        'inline-flex h-9 shrink-0 min-w-max items-center overflow-hidden rounded-[10px] border transition-all duration-150',
                        'bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06),0_1px_2px_rgba(0,0,0,0.04)] dark:bg-zinc-900 dark:shadow-none',
                        open
                            ? 'border-emerald-500/40 ring-2 ring-emerald-500/10 dark:border-emerald-500/30'
                            : activeCount > 0
                              ? 'border-emerald-500/30 hover:border-emerald-500/50 dark:border-emerald-500/20 dark:hover:border-emerald-500/30'
                              : 'border-black/8 hover:border-black/14 dark:border-white/8 dark:hover:border-white/14',
                    ].join(' ')}
                >
                    <span
                        className={[
                            'flex h-full w-9 shrink-0 items-center justify-center rounded-l-[10px] border-r transition-colors duration-150',
                            activeCount > 0
                                ? 'border-emerald-500/20 bg-emerald-500/[0.07] dark:border-emerald-500/15 dark:bg-emerald-500/10'
                                : 'border-black/6 bg-stone-50 dark:border-white/6 dark:bg-white/3',
                        ].join(' ')}
                    >
                        <SlidersHorizontal
                            className={[
                                'h-3.5 w-3.5 transition-colors duration-150',
                                activeCount > 0
                                    ? 'text-emerald-600 dark:text-emerald-400'
                                    : 'text-gray-400 dark:text-gray-500',
                            ].join(' ')}
                        />
                    </span>
                    <span className="flex items-center gap-2 px-3">
                        <span className="text-xs font-medium text-gray-600 dark:text-gray-300">
                            Filters
                        </span>
                        {activeCount > 0 && (
                            <span className="inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-emerald-600 px-1 text-[10px] font-semibold text-white tabular-nums shadow-sm shadow-emerald-600/20 dark:bg-emerald-500">
                                {activeCount}
                            </span>
                        )}
                    </span>
                </button>
            </PopoverTrigger>

            <PopoverContent
                align="end"
                className="w-[calc(100vw-2rem)] overflow-hidden rounded-[14px] border border-black/6 bg-white p-0 shadow-[0_8px_30px_rgba(0,0,0,0.08)] sm:w-72 dark:border-white/6 dark:bg-zinc-900 dark:shadow-[0_8px_30px_rgba(0,0,0,0.4)]"
            >
                <div className="flex items-center justify-between border-b border-black/6 px-4 py-3 dark:border-white/6">
                    <span className="text-[13px] font-medium text-gray-900 dark:text-gray-100">
                        Filters
                    </span>
                    <button
                        onClick={handleClearAll}
                        disabled={!hasActiveFilters}
                        className="inline-flex items-center gap-1 rounded-full px-1.5 py-1 text-[11px] font-medium text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500 disabled:pointer-events-none disabled:opacity-40 dark:text-gray-500 dark:hover:bg-red-500/10 dark:hover:text-red-400"
                    >
                        <X className="h-3 w-3" />
                        Clear all
                    </button>
                </div>

                <div className="max-h-72 space-y-1 overflow-y-auto p-2">
                    <RtsFilterGroup
                        name="PAGE"
                        options={pageOptions}
                        selected={localValue.pageIds}
                        onSelect={(id) => handleSelect('pageIds', id)}
                    />
                    <RtsFilterGroup
                        name="SHOP"
                        options={shopOptions}
                        selected={localValue.shopIds}
                        onSelect={(id) => handleSelect('shopIds', id)}
                    />
                    <RtsFilterGroup
                        name="TEAM"
                        options={teamOptions}
                        selected={localValue.teamIds}
                        onSelect={(id) => handleSelect('teamIds', id)}
                    />
                </div>

                <div className="flex gap-2 border-t border-black/6 px-4 py-3 dark:border-white/6">
                    <button
                        onClick={handleApply}
                        disabled={!hasChanges}
                        className="flex-1 rounded-lg bg-emerald-600 py-2 text-xs font-medium text-white transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-emerald-500 dark:hover:bg-emerald-400"
                    >
                        Apply
                    </button>
                    <button
                        onClick={() => {
                            setLocalValue(value);
                            setHasChanges(false);
                            setOpen(false);
                        }}
                        className="rounded-lg border border-black/6 bg-white px-4 py-2 text-xs font-medium text-gray-500 transition-colors hover:border-black/10 dark:border-white/6 dark:bg-zinc-900 dark:text-gray-400 dark:hover:border-white/10"
                    >
                        Cancel
                    </button>
                </div>
            </PopoverContent>
        </Popover>
    );
}
