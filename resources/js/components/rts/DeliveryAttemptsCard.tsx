...
export default function DeliveryAttemptsCard({
    workspaceSlug,
    queryParams,
}: Props) {
    ...
    useEffect(() => {
        ...
        fetch(
            `/workspaces/${workspaceSlug}/rts/analytics/group-by/delivery-attempts?${p}`,
            ...
            <div className="flex items-center justify-between border-b border-black/6 px-5 py-4 dark:border-white/6">
                <div>
                    ...
                    <p className="mt-0.5 text-[12px] text-gray-400 dark:text-gray-500">
                        RTS rate by number of delivery attempts
                    </p>
                </div>
                ...
                {loading ? (
                    ...
                    <table className="w-full text-[12px]">
                        ...
                        <tbody>
                            {rows.map((row) => (
                                <tr
                                    key={row.delivery_attempts ?? 'unknown'}
