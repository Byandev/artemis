...
export default function SubscriptionExpiredModal() {
    const { subscriptionExpired } = usePage().props as {
        subscriptionExpired?: SubscriptionExpiredData;
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="mx-4 max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl dark:bg-zinc-900">
                <div className="mb-6 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-zinc-500">
                            Your free trial for{' '}
                            <strong>{workspace.name}</strong> has ended. Choose
                            a plan to continue.
                        </p>
                        {current_period_end && (
                            <p className="text-sm text-zinc-500">
                        ...
                        >
                            ...
                            <ul className="mt-4 flex-1 space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
                                <li className="flex items-center gap-2">
                                    ...
                                    {plan.order_limit
                                        ? `${plan.order_limit.toLocaleString()} orders/mo`
                                        : 'Unlimited orders'}
                                </li>
                                <li className="flex items-center gap-2">
                                    <Check className="h-3.5 w-3.5 text-green-500" />
                                ...
                                <li className="flex items-center gap-2">
                                    <Check className="h-3.5 w-3.5 text-green-500" />
                                    {plan.data_retention_months} months
                                    retention
                                </li>
                                <li className="flex items-center gap-2">
                                    <Check className="h-3.5 w-3.5 text-green-500" />
                                ...
                                <li className="flex items-center gap-2">
                                    <Check className="h-3.5 w-3.5 text-green-500" />
                                    {plan.support_tier === 'dedicated'
                                        ? 'Dedicated support'
                                        : plan.support_tier === 'priority_chat'
                                          ? 'Priority chat'
                                          : 'Chat support'}
                                </li>
                            </ul>
                            ...
                            <a
                                href="#"
                                className="mt-5 block w-full rounded-md bg-brand-600 px-4 py-2.5 text-center text-sm font-medium text-white transition-colors hover:bg-brand-700"
                            >
                                Subscribe
                            </a>
                        </div>
