// Transaction types are now DB-backed and workspace-scoped (managed on the
// Transaction Types page). The stored value is a free-form name; there is no
// fixed union any more. Labels are humanized from the name and badge styles
// come from a known-name palette with a neutral fallback for custom types.
...
/** Turn a list of stored type names into {value,label} options for selects. */
export function buildTransactionTypeOptions(
    names: string[],
): TransactionTypeOption[] {
    return names.map((name) => ({
        value: name,
        label: transactionTypeLabel(name),
    }));
}

// Badge styling for the legacy/known type names. Custom types fall back to a
// neutral slate badge so the UI stays consistent without per-type config.
const KNOWN_STYLE: Record<string, { cls: string }> = {
    funds: {
...
/** Badge style for a transaction type value, with a neutral fallback. */
export function transactionTypeStyle(value?: string | null): { cls: string } {
    if (!value) return FALLBACK_STYLE;
    return KNOWN_STYLE[value] ?? FALLBACK_STYLE;
}
