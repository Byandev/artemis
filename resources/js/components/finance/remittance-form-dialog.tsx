...
export function RemittanceFormDialog({
        ...
        if (open) {
            if (remittance) {
                setData({
                    ...
                    billing_date_from: String(
                        remittance.billing_date_from,
                    ).slice(0, 10),
                    billing_date_to: String(remittance.billing_date_to).slice(
                        0,
                        10,
                    ),
                    ...
                    transaction_id: remittance.transaction_id
                        ? String(remittance.transaction_id)
                        : '',
    ...
    useEffect(() => {
        if (!codFeeOverride) {
            const computed = (
                Number(data.gross_cod || 0) * COD_FEE_RATE
            ).toFixed(2);
    ...
    useEffect(() => {
        if (!codFeeVatOverride) {
            const computed = (
                Number(data.cod_fee || 0) * COD_FEE_VAT_RATE
            ).toFixed(2);
                ...
                <div className="border-b border-black/6 px-5 pt-5 pb-4 dark:border-white/6">
                    <DialogHeader>
                        ...
                        <DialogDescription className="mt-0.5 text-[12px] text-gray-400 dark:text-gray-500">
                            COD Fee = Gross × 2.75%, VAT = COD Fee × 12%. Net =
                            Gross − COD Fee − VAT − Shipping − Return Shipping.
                        </DialogDescription>
                    </DialogHeader>
                </div>
                    ...
                    <div className="max-h-[70vh] space-y-5 overflow-y-auto px-5 py-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <Field
                                label="Billing From"
                                required
                                error={errors.billing_date_from}
                            >
                                <input
                                    type="date"
                                    value={data.billing_date_from}
                                    onChange={(e) =>
                                        setData(
                                            'billing_date_from',
                                            e.target.value,
                            ...
                            <Field
                                label="Billing To"
                                required
                                error={errors.billing_date_to}
                            >
                                <input
                                    type="date"
                                    value={data.billing_date_to}
                                    onChange={(e) =>
                                        setData(
                                            'billing_date_to',
                                            e.target.value,
                        ...
                        <Field
                            label="Gross COD"
                            required
                            error={errors.gross_cod}
                        >
                            <input
                                type="number"
                                step="0.01"
                                min="0"
                                value={data.gross_cod}
                                onChange={(e) =>
                                    setData('gross_cod', e.target.value)
                                }
                                className={inputCls}
                            />
                        </Field>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            setCodFeeOverride(true);
                                            setData('cod_fee', e.target.value);
                                        }}
                                        className={inputCls}
                                    />
                                    {codFeeOverride && (
                                        <button
                                            ...
                                            className="shrink-0 font-mono text-[10px] text-gray-400 hover:text-emerald-600"
                                            title="Reset to auto"
                                        >
                                            auto
                                        </button>
                            ...
                            <Field
                                label="COD Fee VAT (12%)"
                                required
                                error={errors.cod_fee_vat}
                            >
                                <div className="flex items-center gap-2">
                                    <input
                                        type="number"
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                                <input
                                    ...
                                    onChange={(e) =>
                                        setData('shipping_fee', e.target.value)
                                    }
                                    className={inputCls}
                                />
                            </Field>
                            <Field
                                ...
                                required
                                error={errors.return_shipping}
                            >
                                <input
                                    type="number"
                                    step="0.01"
                                    min="0"
                                    value={data.return_shipping}
                                    onChange={(e) =>
                                        setData(
                                            'return_shipping',
                                            e.target.value,
