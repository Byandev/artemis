...
export default function RemittancesIndex({
    ...
    const unreconciled =
        query?.filter?.unreconciled === '1' ||
        query?.filter?.unreconciled === 'true';
    ...
    const performQuery = useCallback(
        debounce(
            (s: string, df: string | undefined, dt: string | undefined) => {
                router.get(
                    baseUrl,
                    {
                        sort: query?.sort,
                    ...
                    },
                    {
                        ...
                        replace: true,
                        preserveScroll: true,
                        only: ['remittances', 'unreconciledCount'],
                    },
                );
            },
            400,
    ...
    useEffect(() => {
        performQuery(search, dateFrom, dateTo);
        return () => performQuery.cancel();
    }, [search, dateFrom, dateTo, performQuery]);

    const columns: ColumnDef<Row>[] = [
        {
            accessorKey: 'soa_number',
            enableSorting: true,
            header: ({ column }) => (
                <SortableHeader column={column} title="SOA No." />
            ),
            cell: ({ row }) => (
                <Link
                    href={`${baseUrl}/${row.original.id}`}
                    className="font-mono text-[12px] font-medium text-gray-800 hover:text-emerald-600 dark:text-gray-100"
                >
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <span className="font-mono text-[11px] text-gray-600 dark:text-gray-400">
                    {String(row.original.billing_date_from).slice(0, 10)} →{' '}
                    {String(row.original.billing_date_to).slice(0, 10)}
                </span>
            ),
        },
        {
            accessorKey: 'gross_cod',
            enableSorting: true,
            header: ({ column }) => (
                <SortableHeader
                    column={column}
            ...
            cell: ({ row }) => (
                <div className="text-right font-mono text-[12px] text-gray-700 dark:text-gray-200">
                    {fmt(row.original.gross_cod)}
                </div>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <div className="text-right font-mono text-[12px] text-gray-500">
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="text-right font-mono text-[12px] text-gray-500">
                    {fmt(row.original.return_shipping)}
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="text-center">
                    <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 font-mono text-[10px] uppercase ${row.original.status === 'remitted' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400'}`}
                    >
                        {row.original.status}
                    </span>
                </div>
            ...
            <div className="mx-auto w-full max-w-(--breakpoint-2xl) p-4 md:p-6">
                ...
                {unreconciledCount > 0 && (
                    <div className="mb-4 flex items-center gap-2 rounded-[10px] border border-amber-200 bg-amber-50 px-4 py-2.5 text-[12px] text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/5 dark:text-amber-300">
                        ...
                        {unreconciledCount} remittance(s) not yet linked to a
                        transaction.
                    </div>
                )}
                        ...
                        onFetch={(params) => {
                            router.get(
                                baseUrl,
                                {
                                    ...
                                    'filter[search]': search || undefined,
                                    'filter[unreconciled]': unreconciled
                                        ? 1
                                        : undefined,
                                    'filter[date_from]': dateFrom || undefined,
                                    'filter[date_to]': dateTo || undefined,
