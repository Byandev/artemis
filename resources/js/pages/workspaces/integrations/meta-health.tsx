...
function StatusPill({ status }: { status: string | null }) {
    ...
    > = {
        success: { Icon: CheckCircle2, cls: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400', dot: 'bg-emerald-500', label: 'Success' },
        failed: { Icon: XCircle, cls: 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400', dot: 'bg-red-400', label: 'Failed' },
        rate_limited: { Icon: AlertTriangle, cls: 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400', dot: 'bg-amber-400', label: 'Throttled' },
        running: { Icon: Clock, cls: 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400', dot: 'bg-blue-400', label: 'Running' },
    }
    const c = config[status] ?? config.failed

    return (
        <span className={clsx('inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide', c.cls)}>
        ...
        <div className="rounded-[14px] border border-black/6 bg-white p-5 transition-colors hover:border-black/10 dark:border-white/6 dark:bg-zinc-900 dark:hover:border-white/10">
            <div className="flex items-start justify-between">
                <div>
                    ...
                    <p className={clsx('mt-2 text-2xl font-semibold tracking-tight', tones[tone])}>{value}</p>
                </div>
...
export default function MetaHealthDashboard({
    ...
    const successRate24h = totalRuns24h > 0 ? Math.round((successRuns24h / totalRuns24h) * 100) : 0

    const summaryColumns: ColumnDef<AccountSummary>[] = [
        {
            id: 'account',
            header: ({ column }) => <SortableHeader column={column} title="Ad Account" enabled={false} />,
            cell: ({ row }) => (
        ...
        ...entityTypes.map<ColumnDef<AccountSummary>>((entity) => ({
            id: entity,
            header: ({ column }) => <SortableHeader column={column} title={ENTITY_LABELS[entity] ?? entity} enabled={false} />,
            cell: ({ row }) => {
                const cell = row.original.entities.find((e) => e.entity_type === entity)
    ...
    const recentColumns: ColumnDef<RecentRun>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) =>
                row.original.account_name ? (
                    ...
                    <span className="text-[11px] text-gray-300 italic dark:text-gray-600">workspace-wide</span>
                ),
        },
        {
        ...
        },
        {
            ...
            enableSorting: true,
            header: ({ column }) => <SortableHeader column={column} title="Records" />,
            cell: ({ row }) =>
        ...
        },
        {
            ...
            header: ({ column }) => <SortableHeader column={column} title="Duration" enabled={false} />,
            cell: ({ row }) =>
        ...
        <AppLayout>
            ...
            <div className="w-full space-y-6 p-4 md:p-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <StatCard label="Connected Accounts" value={summary.length} icon={Database} />
                    <StatCard label="Runs (24h)" value={totalRuns24h} icon={Activity} />
                    <StatCard
                        label="Success rate (24h)"
                        value={`${successRate24h}%`}
                        icon={CheckCircle2}
                        tone={successRate24h >= 95 ? 'success' : successRate24h >= 80 ? 'warning' : 'danger'}
                    />
                    <StatCard
                        label="Failed / Throttled (24h)"
                        value={failedLast24h}
                        icon={AlertTriangle}
                        tone={failedLast24h === 0 ? 'success' : failedLast24h < 5 ? 'warning' : 'danger'}
                    />
                </div>
                ...
                {lastSuccessfulRun && (
                    <div className="rounded-[14px] border border-emerald-100 bg-emerald-50/40 px-4 py-3 dark:border-emerald-500/15 dark:bg-emerald-500/5">
                        <p className="font-mono text-[11px] text-emerald-700 dark:text-emerald-400">
                            Last successful run: {ENTITY_LABELS[lastSuccessfulRun.entity_type] ?? lastSuccessfulRun.entity_type}
                            {lastSuccessfulRun.account_name && ` · ${lastSuccessfulRun.account_name}`} · {formatRelative(lastSuccessfulRun.started_at)}
                        </p>
                    </div>
                )}
                ...
                <div>
                    <div className="mb-3 flex items-center justify-between">
                        <h2 className="font-mono text-[10px] font-medium uppercase tracking-[0.08em] text-gray-400 dark:text-gray-500">
                            Per-account status
                        </h2>
                        <span className="font-mono text-[10px] text-gray-300 dark:text-gray-600">
                            {summary.length} account{summary.length === 1 ? '' : 's'}
                        </span>
                    </div>
                            ...
                            onFetch={(params) => {
                                router.get(
                                    indexUrl,
                                    {
                                        sort: params?.sort,
                                        page: params?.page ?? 1,
                                        per_page: params?.per_page ?? query?.perPage ?? recent.per_page,
                                    },
