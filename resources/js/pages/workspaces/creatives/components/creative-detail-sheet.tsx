...
const sectionLabel = 'font-mono text-[9px] font-medium uppercase tracking-widest text-gray-400 dark:text-gray-600';
...
function ReviewerChip({ name }: { name: string }) {
    return (
        <span className="inline-flex items-center gap-1 rounded-full bg-blue-500/[0.08] py-0.5 pr-2 pl-0.5 font-mono text-[11px] font-medium text-blue-700 dark:bg-blue-500/[0.15] dark:text-blue-300">
            <InitialAvatar name={name} className="h-4 w-4 bg-blue-500/20 text-[8px] text-blue-700 dark:text-blue-300" />
            {name}
        </span>
    );
}
...
function TextBlock({ label, children, tone = 'plain' }: { label: string; children: ReactNode; tone?: 'plain' | 'mono' | 'note' }) {
    const body =
        tone === 'note'
...
function ReviewComment({
    ...
    return (
        <div className="relative flex gap-3">
            {!isLast && <div className="absolute top-8 bottom-0 left-3.5 w-px bg-black/6 dark:bg-white/6" />}
                ...
                {editing ? (
                    ...
                    <div className="overflow-hidden rounded-[12px] border border-black/6 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)] dark:border-white/6 dark:bg-zinc-800/40 dark:shadow-none">
                        <div className="flex items-center gap-2 border-b border-black/4 px-3 py-2 dark:border-white/4">
                            <span className="truncate text-[12px] font-semibold text-gray-800 dark:text-gray-200">{review.reviewer?.name ?? 'Unknown'}</span>
                            <ReviewBadge status={review.status} />
                            <time className="ml-auto shrink-0 font-mono text-[10px] text-gray-400 dark:text-gray-600">{review.created_at}</time>
                            {isAuthor && (
                                <button onClick={() => setEditing(true)} title="Edit review" className="shrink-0 rounded-md p-1 text-gray-300 transition-colors hover:bg-stone-100 hover:text-gray-600 dark:text-gray-700 dark:hover:bg-zinc-700 dark:hover:text-gray-400">
                        ...
                        {review.feedback
                            ...
                            : <p className="px-3 py-2.5 font-mono text-[11px] text-gray-300 italic dark:text-gray-700">No feedback left</p>
                        }
                    </div>
                )}
            </div>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div>
                ...
                {creative.assigned_reviewers.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-1.5">
                        {creative.assigned_reviewers.map((r) => <ReviewerChip key={r.id} name={r.name} />)}
                    </div>
                ) : (
                    <p className={muted}>No one is assigned to review this yet.</p>
                )}
            </div>
            ...
            <div>
                <div className="mb-2 flex items-center justify-between">
                    <p className={sectionLabel}>Reviews{creative.review_count > 0 ? ` · ${creative.review_count}` : ''}</p>
                </div>
                {reversed.length === 0 ? (
                    <div className="rounded-[12px] border border-dashed border-black/8 px-6 py-8 text-center dark:border-white/8">
                        ...
                        {canReview && <p className="mt-0.5 font-mono text-[10px] text-gray-300 dark:text-gray-700">Be the first to leave feedback.</p>}
                    </div>
                ) : (
                    <div>
                        {reversed.map((r, i) => (
                ...
                showForm ? (
                    <div className="overflow-hidden rounded-[14px] border border-black/6 bg-stone-50/60 dark:border-white/6 dark:bg-zinc-800/30">
                        <div className="flex items-center justify-between border-b border-black/6 px-4 py-3 dark:border-white/6">
                            ...
                            <button type="button" onClick={closeForm} className="rounded-md p-1 text-gray-300 transition-colors hover:bg-stone-100 hover:text-gray-600 dark:text-gray-700 dark:hover:bg-zinc-700 dark:hover:text-gray-400">
                        ...
                        <form onSubmit={submit} className="space-y-3 p-4">
                            ...
                            <textarea value={data.feedback} onChange={(e) => setData('feedback', e.target.value)} rows={3} autoFocus placeholder="Leave feedback or revision notes..."
                            ...
                            <div className="flex gap-2">
                                <button type="submit" disabled={processing} className="flex h-8 flex-1 items-center justify-center rounded-lg bg-emerald-600 font-mono! text-[12px]! font-medium text-white transition-colors hover:bg-emerald-700 disabled:opacity-50">
                                    Submit Review
                                </button>
                                <button type="button" onClick={closeForm} className="flex h-8 items-center rounded-lg border border-black/8 bg-white px-3 font-mono! text-[12px]! font-medium text-gray-600 transition-colors hover:bg-stone-100 dark:border-white/8 dark:bg-zinc-800 dark:text-gray-300 dark:hover:bg-zinc-700">
                                    Cancel
                                </button>
                            </div>
                        </form>
...
const FORMAT_META: Record<'video' | 'image', { icon: ComponentType<{ className?: string }>; tint: string; fg: string }> = {
    video: { icon: Clapperboard, tint: 'bg-violet-50 dark:bg-violet-500/[0.10]', fg: 'text-violet-500 dark:text-violet-400' },
    image: { icon: FileImage, tint: 'bg-blue-50 dark:bg-blue-500/[0.10]', fg: 'text-blue-500 dark:text-blue-400' },
};
...
export function CreativeDetailSheet({
    ...
    const hasContent = !!(creative.headline || creative.caption || creative.description || creative.script || creative.notes);
                ...
                <Tabs defaultValue="details" className="flex h-full flex-col overflow-hidden">
                    ...
                    <div className="shrink-0">
                        <div className="flex items-start gap-3 border-b border-black/6 pt-5 pr-12 pb-4 pl-5 dark:border-white/6">
                            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-[10px] ${fmt.tint}`}>
                                <FormatIcon className={`h-[18px] w-[18px] ${fmt.fg}`} />
                            </div>
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-gray-400 dark:text-gray-600">
                                    {creative.creator && (
                                        <>
                                            <InitialAvatar name={creative.creator.name} className="h-4 w-4 bg-emerald-500/[0.12] text-[8px] text-emerald-600 dark:text-emerald-400" />
                        ...
                        <div className="divide-y divide-black/4 dark:divide-white/4">
                            ...
                            <Section label="Properties">
                                <dl className="divide-y divide-black/4 dark:divide-white/4">
                                    <Prop icon={Package} label="Product">
                                        {creative.product ? (
                                            <span className="inline-flex max-w-full items-center gap-1.5 rounded-md bg-indigo-50 px-2 py-1 font-mono text-[11px] font-medium text-indigo-600 dark:bg-indigo-500/[0.12] dark:text-indigo-400">
                                                <Package className="h-3 w-3 shrink-0" /> <span className="truncate">{creative.product.title}</span>
                                            </span>
                                        ) : (
                                            <span className={muted}>No product</span>
                                        )}
                                    </Prop>
                                    ...
                                    <Prop icon={Megaphone} label="Ads status"><AdsBadge status={creative.ads_status} /></Prop>
                                    <Prop icon={Flag} label="Final status"><FinalBadge status={creative.final_status} /></Prop>
                                    <Prop icon={MessageSquare} label="Latest review">
                                        {creative.latest_review ? <ReviewBadge status={creative.latest_review.status} /> : <span className={muted}>—</span>}
                                    </Prop>
                                    {creative.approved_at && (
                                        <Prop icon={CalendarCheck} label="Approved">
                                            <span className="font-mono text-[11px] font-medium text-emerald-600 dark:text-emerald-400">{creative.approved_at}</span>
                                        </Prop>
                                    )}
                                </dl>
                            </Section>
                            ...
                            {creative.picture_url && (
                                <Section label="Media">
                                    <a href={creative.picture_url} target="_blank" rel="noopener noreferrer"
                                        ...
                                        <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${fmt.tint}`}>
                                            <FormatIcon className={`h-4 w-4 ${fmt.fg}`} />
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <p className="text-[12px] font-medium text-gray-800 dark:text-gray-200">Open media</p>
                                            <p className="mt-0.5 truncate font-mono text-[10px] text-gray-400 dark:text-gray-500">{creative.picture_url}</p>
                                        </div>
                            ...
                            {hasLinks && (
                                <Section label="Links">
                                    <div className="flex flex-wrap gap-2">
                                        {creative.reference_link && <LinkPill href={creative.reference_link}>Reference</LinkPill>}
                                        {creative.ads_manager_link && <LinkPill href={creative.ads_manager_link}>Ads Manager</LinkPill>}
                                    </div>
                                </Section>
                            )}
                            ...
                            {creative.caption && <TextBlock label="Caption">{creative.caption}</TextBlock>}
                            {creative.description && <TextBlock label="Description">{creative.description}</TextBlock>}
                            {creative.script && <TextBlock label="Script" tone="mono">{creative.script}</TextBlock>}
                            {creative.notes && <TextBlock label="Notes" tone="note">{creative.notes}</TextBlock>}

                            {!hasContent && !hasLinks && !creative.picture_url && (
