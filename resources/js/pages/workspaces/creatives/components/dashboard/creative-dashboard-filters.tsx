...
import { EditorOption, ProductOption } from './types';

/** The multi-select subset of the dashboard filters managed by this popover. */
...
/**
 * Filter popover for the video-editor dashboard, styled to match the main
 ...
 */
export default function CreativeDashboardFilters({
    value,
    editors,
    products,
    defaultUserId,
    ...
    const [localValue, setLocalValue] = useState<CreativeFilterValue>(value);
    const [hasChanges, setHasChanges] = useState(false);

    const editorOptions = useMemo<Option[]>(
        () => editors.map((e) => ({ key: String(e.id), label: e.name })),
        [editors],
    );
    const productOptions = useMemo<Option[]>(
        () => products.map((p) => ({ key: String(p.id), label: p.title })),
        [products],
    );
            ...
            >
                ...
                <div className="max-h-72 space-y-1 overflow-y-auto p-2">
                    ...
                    <MultiSelectGroup
                        ...
                        selected={localValue.product_ids}
                        onToggle={(id) => toggle('product_ids', id)}
                    />
                </div>
