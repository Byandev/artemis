...
export default function CreativesIndex({ workspace, creatives, creators, query }: PageProps) {
    ...
    const detailCreative = detailCreativeId !== null
    ...
    const columns: ColumnDef<Creative>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => <span className="font-mono text-[12px] text-gray-500 dark:text-gray-400">{row.original.creative_date}</span>,
        },
        {
            accessorKey: 'format',
            enableSorting: true,
    ...
    const activeFilterCount = [query.filter?.format, query.filter?.ads_status, query.filter?.creator_id].filter(Boolean).length;
                ...
                <div className="mb-3 flex items-center gap-2">
                    ...
                    <DatePicker
                        ...
                        onChange={(dates) => {
                            if (dates.length === 2) {
                                ...
                                navigate({ 'filter[date_from]': fmt(dates[0]), 'filter[date_to]': fmt(dates[1]), page: 1 });
                            } else if (dates.length === 0) {
                                navigate({ 'filter[date_from]': undefined, 'filter[date_to]': undefined, page: 1 });
                            }
                        }}
                    />
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="w-52 p-2" align="end">
                            <p className="mb-1 px-2 font-mono text-[9px] font-medium uppercase tracking-widest text-gray-400 dark:text-gray-600">Format</p>
                            {([
                                { key: '', icon: null, label: 'All formats' },
                                { key: 'video', icon: <Clapperboard className="h-3 w-3" />, label: 'Video' },
                                { key: 'image', icon: <FileImage className="h-3 w-3" />, label: 'Image' },
                            ] as { key: string; icon: React.ReactNode; label: string }[]).map(({ key, icon, label }) => (
                                <button key={key} className={filterItem((query.filter?.format ?? '') === key)} onClick={() => navigate({ 'filter[format]': key || undefined, page: 1 })}>
                                    {icon ?? <span className="h-3 w-3" />}{label}
                                </button>
                            ))}

                            <DropdownMenuSeparator className="my-2" />

                            <p className="mb-1 px-2 font-mono text-[9px] font-medium uppercase tracking-widest text-gray-400 dark:text-gray-600">Ads Status</p>
                            <button className={filterItem(!query.filter?.ads_status)} onClick={() => navigate({ 'filter[ads_status]': undefined, page: 1 })}>
                                <span className="h-3 w-3" />All statuses
                            </button>
                            {(Object.entries(ADS_STATUS_LABELS) as [AdsStatus, string][]).map(([v, l]) => (
                            ...
                            {creators.length > 0 && (
                                <>
                                    <DropdownMenuSeparator className="my-2" />
                                    <p className="mb-1 px-2 font-mono text-[9px] font-medium uppercase tracking-widest text-gray-400 dark:text-gray-600">Creator</p>
                                    <button className={filterItem(!query.filter?.creator_id)} onClick={() => navigate({ 'filter[creator_id]': undefined, page: 1 })}>
                                        <span className="h-3 w-3" />All creators
                                    </button>
                                    {creators.map((c) => (
                                        <button key={c.id} className={filterItem(query.filter?.creator_id === String(c.id))} onClick={() => navigate({ 'filter[creator_id]': String(c.id), page: 1 })}>
                                            <span className="flex h-3 w-3 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 font-mono text-[8px] font-bold text-emerald-600 dark:text-emerald-400">{c.name.charAt(0).toUpperCase()}</span>
                            ...
                            {activeFilterCount > 0 && (
                                <>
                                    <DropdownMenuSeparator className="my-2" />
                                    <button className="flex w-full items-center justify-center rounded-[8px] px-2.5 py-1.5 font-mono text-[11px] text-red-500 transition-colors hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/[0.08]"
                                        onClick={() => navigate({ 'filter[format]': undefined, 'filter[ads_status]': undefined, 'filter[creator_id]': undefined, page: 1 })}>
                                        Clear all filters
                                    </button>
                                </>
                            )}
            ...
            <Dialog open={deleteId !== null} onOpenChange={(v) => !v && setDeleteId(null)}>
                <DialogContent className="gap-0 overflow-hidden border-none p-0 shadow-2xl sm:max-w-sm dark:bg-zinc-900 [&_[data-default-close=true]]:hidden">
                    <div className="relative border-b border-black/6 px-5 pt-5 pb-4 dark:border-white/6">
                        <DialogHeader>
                            <DialogTitle className="text-[15px] font-semibold text-gray-900 dark:text-gray-100">Delete Creative</DialogTitle>
                        </DialogHeader>
                    ...
                    <div className="px-5 py-4">
                        <p className="font-mono text-[12px] text-gray-500 dark:text-gray-400">Permanently deletes the creative along with all reviews and campaign data.</p>
                    </div>
                    <div className="flex items-center justify-end gap-2 border-t border-black/6 bg-stone-50/50 px-5 py-3 dark:border-white/6 dark:bg-white/2">
                        <button type="button" onClick={() => setDeleteId(null)} className="flex h-9 items-center rounded-lg border border-black/8 bg-white px-4 font-mono! text-[12px]! font-medium text-gray-600 transition-all hover:bg-stone-100 dark:border-white/8 dark:bg-zinc-800 dark:text-gray-300 dark:hover:bg-zinc-700">Cancel</button>
                        <button type="button" onClick={() => deleteId !== null && handleDelete(deleteId)} className="flex h-9 items-center rounded-lg bg-red-600 px-4 font-mono! text-[12px]! font-medium text-white transition-all hover:bg-red-700">Delete</button>
                    </div>
                </DialogContent>
            </Dialog>
        </AppLayout>
