...
export default function TopAssignees({ workspace }: Props) {
    const [data, setData] = useState<TopAssignee[]>([]);
    ...
    useEffect(() => {
        ...
        fetch(
            `/workspaces/${workspace.slug}/rts/rmo-management/analytics/top-assignees`,
    ...
    const series = useMemo(
        () => [{ name: 'Assigned Orders', data: data.map((d) => d.total_orders) }],
        [data],
    );
        ...
        <div className="rounded-[14px] border border-black/6 bg-white p-5 dark:border-white/6 dark:bg-zinc-900">
            <div className="mb-4">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                    Top 10 Users by Assigned Orders
                </h3>
                <p className="text-[11px] text-gray-400 dark:text-gray-500">
                    Users with the most assigned orders for delivery
                </p>
            </div>
