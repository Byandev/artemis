...
const BreakdownPerPages = ({ workspace, queryString }: Props) => {
        ...
        const fetchData = async () => {
            ...
            try {
                const res = await fetch(
                    `/workspaces/${workspace.slug}/rts/analytics/group-by/pages${queryString ? `?${queryString}` : ''}`,
    ...
    return (
        <div className="rounded-xl border bg-card p-6 shadow-sm h-fit">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h3 className="text-lg font-semibold tracking-tight">Breakdown per Pages</h3>
                    <p className="text-sm text-muted-foreground">View RTS rate performance across all pages</p>
                </div>
...
export default BreakdownPerPages;
