        ...
        const fetchData = async () => {
            ...
            try {
                const res = await fetch(
                    `/workspaces/${workspace.slug}/rts/analytics/group-by/users${queryString ? `?${queryString}` : ''}`,
                ...
                console.error('Error fetching users breakdown:', error);
            } finally {
                setLoading(false);
            }
        };
        ...
        return (
            <div className="rounded-xl border bg-card p-6 shadow-sm w-full">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold tracking-tight">Breakdown per Users</h3>
                </div>
    ...
    return (
        <div className="rounded-xl border bg-card p-6 shadow-sm h-fit">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h3 className="text-lg font-semibold tracking-tight">Breakdown per Users</h3>
                    <p className="text-sm text-muted-foreground">Monitor RTS performance by user</p>
                </div>
...
export default BreakdownPerUsers;
