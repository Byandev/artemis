...
interface Props {
    ...
    onApply: (value: SalesFilterValue) => void;
}

// Multi-select collapsible group, mirroring the main dashboard's FilterGroup.
...
/**
 * Grouped filter popover for the Daily Sales Tracker, styled to match the main
 * dashboard's Filters control (sliders trigger + grouped multi-select sections
 * + an explicit "Apply Changes" action and a "Clear all" reset).
 */
export default function DailySalesFilters({
    ...
    const [localValue, setLocalValue] = useState<SalesFilterValue>(value);
    const [hasChanges, setHasChanges] = useState(false);

    const activeCount =
        (value.csr.length > 0 ? 1 : 0) +
        (value.platform.length > 0 ? 1 : 0) +
        (value.parcel_status.length > 0 ? 1 : 0) +
        (value.order_status.length > 0 ? 1 : 0);

    const toggle = useCallback((key: keyof SalesFilterValue, id: string) => {
        setLocalValue((prev) => {
    ...
    const handleOpenChange = useCallback(
        (open: boolean) => {
            if (!open) {
                ...
                // Sync staged state with the latest applied value on open.
                setLocalValue(value);
            }
            setIsOpen(open);
        },
            ...
            >
                ...
                <div className="max-h-72 space-y-1 overflow-y-auto p-2">
                    <MultiSelectGroup
                        name="CSR"
                        options={csrs}
                        selected={localValue.csr}
                        onToggle={(id) => toggle('csr', id)}
                    />
                    <MultiSelectGroup
                        name="Platform"
                        options={platforms}
                        selected={localValue.platform}
                        onToggle={(id) => toggle('platform', id)}
                    />
                    <MultiSelectGroup
                        name="Parcel Status"
                        options={parcelStatuses}
                        selected={localValue.parcel_status}
                        onToggle={(id) => toggle('parcel_status', id)}
                    />
                    <MultiSelectGroup
                        name="Order Status"
                        options={orderStatuses}
                        selected={localValue.order_status}
                        onToggle={(id) => toggle('order_status', id)}
                    />
                </div>
