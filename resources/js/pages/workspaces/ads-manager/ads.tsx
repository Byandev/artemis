...
import {
    Ad,
    AVAILABLE_AD_METRICS,
    MetricFilter,
    PaginatedAds,
} from '@/types/models/AdManager';
...
const AdsPage = ({ workspace, ads, query }: PageProps) => {
    ...
    const getNavParams = (overrides: Record<string, any> = {}) => ({
        ...
        metric_filters:
            metricFilters.length > 0
                ? encodeURIComponent(JSON.stringify(metricFilters))
                : undefined,
    ...
    const handleMetricsChange = (metrics: string[]) => {
        ...
        const filteredMetricFilters = metricFilters.filter((filter) =>
            metrics.includes(filter.metric),
        );
        ...
        router.get(
            ...
            getNavParams({
                ...
                metric_filters:
                    filteredMetricFilters.length > 0
                        ? encodeURIComponent(
                              JSON.stringify(filteredMetricFilters),
                          )
                        : undefined,
            }),
            {
    ...
    const columns: ColumnDef<Ad>[] = [
        {
            accessorKey: 'name',
            header: ({ column }) => (
                <SortableHeader column={column} title="Ad Name" />
            ),
            cell: ({ row }) => (
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div>{row.original.campaign?.name || 'N/A'}</div>
            ),
        },
        {
            ...
            ? [
                  {
                      ...
                      cell: ({ row }: { row: any }) =>
                          Number(row.original.clicks || 0).toLocaleString(),
                  } as ColumnDef<Ad>,
              ]
            : []),
            ...
            ? [
                  {
                      accessorKey: 'spend',
                      header: ({ column }: { column: any }) => (
                          <SortableHeader column={column} title="Spend" />
                      ),
                      cell: ({ row }: { row: any }) =>
                          `₱${Number(row.original.spend || 0).toFixed(2)}`,
                  } as ColumnDef<Ad>,
              ]
            : []),
    ];
