...
export const ConditionsBadge = ({ conditions }: ConditionsBadgeProps) => {
    if (!conditions || conditions.length === 0) {
        return (
            <div className="flex items-center gap-2.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-gray-500 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-400">
        ...
        <div className="flex flex-wrap gap-2">
            {conditions.map((condition, idx) => {
                ...
                return (
                    <div
                        key={idx}
                        className={`inline-flex items-center gap-2 rounded-lg bg-linear-to-r px-3 py-1.5 ${colors.bg} ${colors.text} border ${colors.border} cursor-default whitespace-nowrap`}
                    >
                        <Zap className="h-3.5 w-3.5 opacity-70" />
                        <span className="text-xs font-semibold">
                            {startCase(condition.metric)}
                        </span>
                        <span className="text-xs font-medium opacity-80">
                            {startCase(condition.operator)}
                        </span>
