...
export default function Index({ plans, filters }: Props) {
    ...
    const columns: ColumnDef<SubscriptionPlan>[] = [
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-zinc-500 font-mono tracking-tighter">{row.original.code}</div>
                    </div>
                </div>
            ),
        },
        {
            accessorKey: 'price_php', enableSorting: true,
            header: ({ column }) => <SortableHeader column={column} title="Price" />,
            cell: ({ row }) => (
                <div>
                    <div className="font-medium text-zinc-900 dark:text-zinc-100">
                        {parseFloat(row.original.price_php) === 0 ? 'Free' : `₱${parseFloat(row.original.price_php).toLocaleString()}`}
                    </div>
                    {row.original.trial_days ? (
                        <div className="text-xs text-zinc-500">{row.original.trial_days}-day trial</div>
                    ) : null}
                </div>
            ),
        },
        {
            id: 'limits', enableSorting: false,
            header: () => <div className="text-center text-zinc-500 uppercase text-[11px] font-bold tracking-wider">Limits</div>,
            cell: ({ row }) => (
                <div className="text-center text-xs text-zinc-600 dark:text-zinc-400">
                    <div>{row.original.order_limit ? `${row.original.order_limit.toLocaleString()} orders` : 'Unlimited orders'}</div>
                    <div>{row.original.page_limit ? `${row.original.page_limit} pages` : 'Unlimited pages'}</div>
                </div>
            ),
        },
        {
            accessorKey: 'subscriptions_count', enableSorting: true,
            header: ({ column }) => <SortableHeader column={column} title="Subscribers" className="justify-center" />,
            cell: ({ row }) => (
        ...
        {
            accessorKey: 'is_active', enableSorting: true,
